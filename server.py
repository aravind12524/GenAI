...
def register():
    ...
    for q_set in question_sets:
        all_questions.extend(q_set.copy())
    
    # Shuffle all questions
    random.shuffle(all_questions)
    
    # Create new randomized set of 10 questions
    randomized_set = all_questions[:10]
    ...
    players[username] = {
        ...
        "answered_ids": set(),
        "attempted_scores": {},
        "current_set": randomized_set,  # Store the randomized set
...
def evaluate():
    try:
        ...
        if code.strip() == challenge_code.strip():
            ...
            return jsonify({
                "feedback": "⚠️ No changes made - You submitted the original challenge code verbatim (5/10 for base correctness)",
                "score": 5,
        ...
        if not user_func or user_func != challenge_func:
            return jsonify({
                "feedback": f"Function name must be '{challenge_func}'",
                "score": 0,
                "total_score": player["score"]
            })
        ...
        try:
            ...
            for args, expected in test_cases[test_case_key]:
                ...
                if result != expected:
                    return jsonify({
                        "feedback": f"Test failed for input {args}",
                        "score": 0,
                        "total_score": player["score"]
                    })
...
def _get_ai_optimized_code(code):
    prompt = """Optimize this Python code with these requirements:
1. Never return identical code
2. Must improve either complexity, speed, or memory
3. Use best optimization techniques
...
def _calculate_score(user_code, challenge_code, ai_code, user_complexity, challenge_complexity):
    ...
    if user_code.strip() == challenge_code.strip():
        return 5, "🔍 Submitted original code (5/10 for correctness - try optimizing for higher scores!)"
    ...
    if user_code.strip() == ai_code.strip():
        ...
        return 9, f"🌟 Excellent! Improved complexity ({user_complexity} vs original {challenge_complexity})"
    elif user_rank == challenge_rank:
        if len(user_code.splitlines()) < len(challenge_code.splitlines()):
            return 8, f"💡 Great! Same complexity but more concise ({user_complexity})"
